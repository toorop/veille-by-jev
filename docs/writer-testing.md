...
{"items": [{"id": "hn:49731360",
            "titre_fr": "the article's own title, in French",
...
`titre_fr` is a **translation of the title, not a summary of it**: faithful, no longer than the
original, product names and model numbers kept as they are. The digest uses it as the item's
heading, as a link to the article, and prints the English original on a `Titre original` line
underneath. A model that omits the field loses nothing: the English title stands alone and the
`Titre original` line disappears. A model that embellishes it is harder to catch than a bad
summary, though, because the heading is what the reader trusts first.
